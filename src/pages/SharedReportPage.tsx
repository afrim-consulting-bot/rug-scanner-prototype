      ...
      {expanded ? (
        <div className="space-y-3">
          {report.checks.map((c) => (
            ...
            >
              <summary className="flex cursor-pointer list-none items-start gap-3 rounded-xl px-4 py-3 transition hover:bg-white/5">
                <div className="min-w-0 flex-1 space-y-1 pr-1">
                  <div className="text-sm font-semibold text-white">{c.title}</div>
                  <div className={`text-xs ${statusClasses(c.status)}`}>{statusLabel(c.status)} — {c.short}</div>
                </div>
                <div className="w-8 flex-shrink-0 self-start pt-0.5 text-right text-slate-400 transition group-open:rotate-180">▾</div>
              </summary>
